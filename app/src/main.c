...
void touch_control_cmd_rsp(uint8_t cmd, uint8_t * rsp) {
  ...
  i2c_transfer(dev_i2c, cmd_rsp_msg, 2, ADDR);
}
...
int main(void) {
  ...
  if(0 > i2c_configure(dev_i2c, I2C_SPEED_SET(I2C_SPEED_FAST) | I2C_MODE_CONTROLLER)) {
    return 0;
  }
  ...
  while(1) {
    ...
    if (touch_status == 1) {
      uint8_t x_pos_h;
      uint8_t x_pos_l;
      uint8_t y_pos_h; // Upper 4 of 12 bits
      uint8_t y_pos_l; // Lower 8 of 12 bits
