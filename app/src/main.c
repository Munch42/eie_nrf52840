...
static void ble_on_advertisement_received(const bt_addr_le_t* addr, int8_t rssi, uint8_t type,
  ...
  char addr_char[BT_ADDR_LE_STR_LEN];
  bt_addr_le_to_str(addr, addr_char, sizeof(addr_char));
  ...
  if (err) {
    bt_conn_unref(my_connection);
    my_connection = NULL;
    printk("Bluetooth connection failed (err %d)\n", err);
    return;
  }
}
...
int main(void) {
  ...
  if (err) {
    printk("Bluetooth Scan Failed (err %d)\n", err);
    return 0;
  }
