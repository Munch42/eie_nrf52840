...
#define BLE_CUSTOM_CHARACTERISTIC_UUID BT_UUID_128_ENCODE(0x12344242, 0x4242, 0x5678, 0x1234, 0x56789abcdef0)
#define BLE_CUSTOM_CHARACTERISTIC_UUID2 BT_UUID_128_ENCODE(0x12344242, 0x4242, 0x5678, 0x1234, 0x56789abcdef1)
...
static ssize_t ble_custom_characteristic2_read_cb(struct bt_conn* conn, const struct bt_gatt_attr* attr,
                                       void* buf, uint16_t len, uint16_t offset);
static ssize_t ble_custom_characteristic2_write_cb(struct bt_conn* conn, const struct bt_gatt_attr* attr,
                                        const void* buf, uint16_t len, uint16_t offset,
                                        uint8_t flags);
...
static uint8_t ble_custom_characteristic_user_data[BLE_CUSTOM_CHARACTERISTIC_MAX_DATA_LENGTH + 1] = {'H', 'i'};
static uint8_t ble_custom_characteristic_user_data2[BLE_CUSTOM_CHARACTERISTIC_MAX_DATA_LENGTH + 1] = {'H', 'o', 'l', 'a'};
...
static const struct bt_uuid_128 ble_custom_characteristic_uuid2 = BT_UUID_INIT_128(BLE_CUSTOM_CHARACTERISTIC_UUID2);
...
int main(void) {
  ...
  int bt_check = bt_enable(NULL);
  if (0 != bt_check) {
    printk("BT Enable Error: %d", bt_check);
    return 0;    
  }

  bt_check = bt_le_adv_start(BT_LE_ADV_CONN_FAST_1, ble_advertising_data, ARRAY_SIZE(ble_advertising_data), NULL, 0);
  if (0 != bt_check) {
    printk("BT Advertising Error: %d", bt_check);
    return 0;
  }
